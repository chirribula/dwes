       ...
       <div id="contenido">
           ...
           <div id="entradas">
                ...
                <?php
                     ...
                     if($buscar != ""){               
                            $sql="SELECT e.titulo,e.id, e.descripcion, c.nombre,e.fecha FROM entradas e, categorias c WHERE e.categoria_id=c.id  AND  e.titulo like '%$buscar%' ORDER BY e.fecha DESC LIMIT 4";           
                ...
                <?php         
                      ...
                      $sql="SELECT e.titulo, e.descripcion,e.id, c.nombre,e.fecha FROM entradas e, categorias c WHERE e.categoria_id=c.id ORDER BY e.fecha DESC LIMIT 4";  
                      $result = mysqli_query($conexion,$sql);
  
                        if(mysqli_num_rows($result)>0){
                            while($fila = mysqli_fetch_assoc($result)){  ?>

                            <span class="trabajo"><?=$fila['titulo']?></span> <?php 
                              if(isset($_SESSION['usuario'])){   
                                ?>  
                              <a class="fa fa-trash" href="borrar.php?borrarId=<?=$fila['id']?>"></a>
                              <a class="fa fa-edit" href="modificar.php?modificarId=<?=$fila['id']?>"></a> 
